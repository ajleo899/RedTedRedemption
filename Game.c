// Game.c
// This file runs the base space invaders game
...
#define FIRE_DELAY = 6;						// delay for enemy fire rate
...
// Sprite used for bunkers used for player cover
struct BunkerSprite{
	uint32_t xPos;									// x-axis position
	uint32_t yPos;									// y-axis position
	uint32_t hp;										// health of object
...
void SpaceInvader_Init(void){
	...
	enemiesAlive = 18;													// enemies are all alive
	enemiesKilled = 0;													// no enemies are dead
	enemyFireDelay = RandomNumber(60);					// random rate of fire for enemy 
	...
	for(; k < 3; k++){													// initializes bunkers
		...
		Bunkers[k].yPos = 151;					// default bunker position
	...
	k = 0;
	for(; k < 4; k++){													// initializes enemy lasers
...
void PlayerXMove(void){
	...
	uint32_t analogIn;																							 // raw ADC data 
	uint32_t data;																									 // converted ADC data
	analogIn = ADC_In();																						 //	read in ADC data
	data = Convert(analogIn);																				 // Convert ADC data using linear regresion to 0 - 2000 value
	data /= 16.5;																										 // divide to fit screen boundaries
	Player.xPos = data;																							 // write position directly to player ship
}
*/
...
/*
// inverted PlayerXMove control scheme
void PlayerXMove(void){
	ST7735_DrawBitmap(Player.xPos, Player.yPos, BlackPlayer, 12, 8); 
	uint32_t analogIn;
	analogIn = ADC_In();
	if((analogIn < 1850) && (Player.xPos < 117)){					
		Player.xPos += 1;																	// move right
	}
	else if((analogIn < 1850) && (Player.xPos >= 5)){	
		Player.xPos = 117;																// maximum right position
	}
	else if((analogIn > 2250) && (Player.xPos > 5)){	
		Player.xPos -= 1;																	// move left
	}
	else if((analogIn > 2250) && (Player.xPos <= 117)){	
		Player.xPos = 0;																	// minimum left position
...
void EnemyMovement(void){
	if(horizontalCounter < 30){
		EnemyMoveRight();						// move enemies right 32 times
		horizontalCounter++;				// increment counter
		frame ^= 1;									// toggle to cue animation
	}
	else if(horizontalCounter < 59){
		EnemyMoveLeft();						// move enemies left 32 times
		horizontalCounter++;				// increment counter
		frame ^= 1;									// toggle to cue animation
	}
	else if(horizontalCounter == 59){
		EnemyMoveLeft();						// move enemy left 1 time
...
void EnemyLaserMove(void){
	int k = 0;
	for(;k < 4; k++){																		// checks state of every enemy laser
		if(EnemyLasers[k].dead == 0){											// if enemy laser is alive
			if(EnemyLasers[k].yPos < 180){									// checks if enemy laser has reached bottom of screen
...
void DrawPauseScreen(void){
  ...
  ST7735_OutString("Are you going to");
  ST7735_SetCursor(1, 5);
  ST7735_OutString("let big bad Ted");
	ST7735_SetCursor(1, 6);
	ST7735_OutString("beat you?");
...
void PlayerFire(void){
	if(playerLaserCount < 1){																		// does not exceed max # of player lasers
		...
		playerLaserCount++;																				// increment count of player lasers
	}
}

/* HAS BUGS, FIX RANDOM FIRING SCHEME, LAST ALIENS DO NOT FIRE
EnemyFire
input: none
output: none
...
void EnemyFire(void){
	...
	//debugDelay = enemyDelay;			// DEBUG VARIABLE, REMOVE ME
	
	if(enemyDelay > 0){						// random delay randomizes enemy firing scheme
		enemyDelay--;								// decrement delay and return
		return;
	}
	else{
		int check;									// new randomized delay
	...
	for(; k < 18; k++){
		...
		//debugFire = fire;						// DEBUG VARIABLE, REMOVE ME
		if((Enemies[k].hp > 0) && (enemyLaserCount < 4) && (fire == 1)){	// fire if enemy is alive, there are less than max # of enemy lasers on screen, and boolean allows fire 
			...
			//enemyLaserCount = debugLaser1;																	// DEBUG VARIABLE, REMOVE ME
			...
			EnemyLasers[k].speed = 2;																				// sets speed of laser
				...
				if((PlayerLasers[k].dead == 0)&&
					...
					if(Enemies[j].hp > 0){			
						...
						if(Enemies[j].hp == 0){		
							...
							Sound_EnemyDestroyed();			// play enemy destroyed sound
						}
					}
...
void BunkerEnemyHitReg(void){
	...
	for(; j < 3; j++){								// check each bunker for:
		if(Bunkers[j].hp > 0){					// if bunker is alive
			...
			for(;k < 4; k++){							// check each enemy laser for:
				if((EnemyLasers[k].dead == 0)&&
